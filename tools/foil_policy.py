"""FOIL vNext V2 evidence-gated runtime policy.

Ported from `experiments/foil_vnext/runtime_policy_v2.py` at `9540860`
(`origin/experiment/foil-vnext5-vnext`). The port is verbatim in substance: the
enums, dataclasses, thresholds, `RuntimePolicyV2` methods and `policy_trace`
are the ported originals. Only this docstring and the module name differ, so a
diff against the source ref stays reviewable.

**V2 mechanisms are implemented hypotheses. Their efficacy is NOT MEASURED.**
This module establishes an executable deterministic policy and its
negative-control invariants; it establishes nothing about whether
profile-driven complements improve any outcome. The production recommendation
stays `VALID_IMPLEMENTATION / BEHAVIORAL_EFFICACY_OPEN` until prospective
paired evidence at matched total cost shows benefit.
...
The `ComplementKind` / gap vocabulary here is the V2 kernel's own and is
deliberately kept separate from `foil_interventions.GAP_KINDS`: the two
vocabularies are not identical, and no cross-mapping is asserted anywhere
