...
_ALLOWED_CHALLENGE_TRANSITIONS: dict[str, frozenset[str]] = {
    "PROPOSED": frozenset({"SELECTED", "UNRESOLVED", "UNAVAILABLE", "RESOLVED", "DISMISSED_NOT_APPLICABLE"}),
    "SELECTED": frozenset({"RUNNING", "UNRESOLVED", "UNAVAILABLE", "RESOLVED", "DISMISSED_NOT_APPLICABLE"}),
    "RUNNING": frozenset({"UNRESOLVED", "UNAVAILABLE", "RESOLVED"}),
    "UNRESOLVED": frozenset({"SELECTED", "RUNNING", "UNAVAILABLE", "RESOLVED", "DISMISSED_NOT_APPLICABLE"}),
    "UNAVAILABLE": frozenset({"SELECTED", "RUNNING", "UNRESOLVED", "RESOLVED", "DISMISSED_NOT_APPLICABLE"}),
    "RESOLVED": frozenset(),
...
class RuntimeStore:
    def __init__(self, root: Path, state_dir: str | None = None) -> None:
        ...
        self.challenge_resolutions = ensure_private_dir(self.base / "challenge_resolutions")
    ...
    def read_task(self, task_id: str, *, require_integrity: bool = True) -> dict[str, Any] | None:
        return self._read(self.tasks / f"{task_id}.json", require_integrity=require_integrity)
    ...
    def write_receipt(self, receipt: Receipt) -> Path:
        ...
        self.append_event(RuntimeEvent(
            ...
            metadata={"receipt_id": receipt.receipt_id, "obligation_id": receipt.obligation_id,
                      "verdict": receipt.verdict.value, "action": receipt.action},
        ))
        self.append_event(RuntimeEvent(
    ...
    def read_receipt(self, receipt_id: str, *, require_integrity: bool = True) -> dict[str, Any] | None:
        return self._read(self.receipts / f"{receipt_id}.json", require_integrity=require_integrity)
    ...
    def write_challenge(self, challenge: Any) -> Path:
        ...
        self._append_challenge_event("challenge.proposed", body, metadata={
            "origin": body.get("origin"), "kind": body.get("kind"),
            "load_bearing": body.get("load_bearing"), "candidate_hash": body.get("candidate_hash"),
            "scope_hash": body.get("scope_hash"), "proposer_provenance": body.get("proposer_provenance"),
        })
        return written

    def read_challenge(self, challenge_id: str, *, require_integrity: bool = True) -> dict[str, Any] | None:
        return self._read(self.challenges / f"{challenge_id}.json", require_integrity=require_integrity)
    ...
    def update_challenge_state(
        ...
        *,
        ...
        with self.lock(f"challenge-{challenge_id}"):
            ...
            if state != current and state not in _ALLOWED_CHALLENGE_TRANSITIONS.get(current, frozenset()):
        ...
        if event_type and emit_event:
            ...
            self._append_challenge_event(event_type, body, component=component, metadata=metadata)
        return written
    ...
    def write_challenge_resolution(self, resolution: Any) -> Path:
        ...
        if str(body.get("state")) not in {
            "RESOLVED", "UNRESOLVED", "UNAVAILABLE", "DISMISSED_NOT_APPLICABLE",
        }:
            raise ValueError("resolution state must be terminal, unresolved, unavailable, or dismissed")
        ...
        with self.lock(f"challenge-resolution-{challenge_id}"):
            ...
            if receipt_id is not None:
                ...
                if body.get("evidence_hash") is not None and body.get("evidence_hash") != receipt_hash:
                    raise ValueError("resolution evidence_hash must bind the linked receipt")
                body["evidence_hash"] = receipt_hash
            elif body.get("state") == "RESOLVED":
                raise ValueError("resolved challenge requires a claim-native verifier receipt")
            body["stored_at"] = utcnow()
            body["seq"] = self.next_seq()
            body["content_hash"] = digest(body)
            written = self._write(path, body)
            self.update_challenge_state(
                challenge_id,
                str(body["state"]),
    ...
    def latest_resolution(self, challenge_id: str, *, valid_only: bool = True) -> dict[str, Any] | None:
        rows: list[dict[str, Any]] = []
    ...
    def _append_challenge_event(
        ...
        *,
        ...
        return self.append_event(RuntimeEvent(
            event_id=new_id("evt"),
