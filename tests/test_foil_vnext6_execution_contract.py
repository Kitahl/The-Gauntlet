...
class ExecutionContractTests(unittest.TestCase):
    ...
    def test_02_internal_operator_cannot_resolve_claim(self):
        ...
        self.assertIn(
            "non_verifying_operator_resolved_claim",
            validation.errors,
        )
    ...
    def test_03_react_observation_does_not_gain_verifier_authority(self):
        ...
        self.assertFalse(validation.valid)
        self.assertIn(
            "react_discovery_claimed_verifier_authority",
            validation.errors,
        )

    def test_04_matching_claim_native_packet_admits_resolution(self):
        decision = source_decision()
        request = build_request(decision, target_claim_ids=("C1",))
        ...
        self.assertEqual(validation.admitted_resolved_claim_ids, ("C1",))
        self.assertEqual(
            validation.progress,
    ...
    def test_05_wrong_verifier_cannot_support_resolution(self):
        ...
        self.assertFalse(validation.valid)
        self.assertIn(
            "unsupported_claim_resolution:C1",
            validation.errors,
        )
    ...
    def test_09_side_effecting_request_requires_idempotency_key(self):
        decision = POLICY.decide(StrategyTaskContext(TaskContext()), BUDGET)
        with self.assertRaises(ValueError):
            build_request(decision, tool_effect=ToolEffect.SIDE_EFFECTING)
    ...
    def test_11_side_effecting_completion_requires_postcondition(self):
        ...
        self.assertFalse(validation.valid)
        self.assertIn(
            "side_effect_postcondition_unverified",
            validation.errors,
        )
    ...
    def test_13_mastermind_requires_distinct_defect(self):
        task = TaskContext(
            has_viable_candidate=True,
            uncertainties=(
                LoadBearingUncertainty("C1", ClaimKind.LOGICAL),
            ),
            completed_verifiers=frozenset(
        ...
        self.assertFalse(validation.valid)
        self.assertIn(
            "mastermind_completed_without_distinct_defect",
            validation.errors,
        )
    ...
    def test_14_mastermind_defect_is_progress_but_not_claim_resolution(self):
        ...
        self.assertTrue(validation.valid, validation.errors)
        self.assertIs(validation.progress, ProgressStatus.PROGRESSED)
        self.assertEqual(validation.admitted_resolved_claim_ids, ())
    ...
    def test_15_reflection_requires_candidate_change(self):
        ...
        self.assertFalse(validation.valid)
        self.assertIn(
            "reflection_completed_without_candidate_change",
            validation.errors,
        )
    ...
    def test_16_terminal_operator_cannot_report_state_change(self):
        ...
        self.assertFalse(validation.valid)
        self.assertIn(
            "terminal_operator_reported_state_change",
            validation.errors,
        )
